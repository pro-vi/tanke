...
# Tank-attached reload pip mirrors top-left bar's progress + shell color
# at combat focus. Faint when ready, bright while filling.
check-breach-reload-pip:
	@$(HEADLESS) --script res://loop/breach/test_breach_reload_pip.gd 2>&1 | grep -E "^(  (procedural|case|idle|mid-cooldown|shell-cycle)|BREACH_RELOAD_PIP_OK|FAIL|ERROR|SCRIPT ERROR)"; \
	$(HEADLESS) --script res://loop/breach/test_breach_reload_pip.gd 2>&1 | grep -q "^BREACH_RELOAD_PIP_OK"
