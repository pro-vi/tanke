...
# grant used to silently swallow picks (last panel overwrites earlier).
# Backlog counter + re-show on _pick_levelup_card drains the queue.
check-breach-multi-levelup-pick:
	@$(HEADLESS) --script res://loop/breach/test_breach_multi_levelup_pick.gd 2>&1 | grep -E "^(  case|BREACH_MULTI_LEVELUP_PICK_OK|FAIL|ERROR|SCRIPT ERROR)"; \
	$(HEADLESS) --script res://loop/breach/test_breach_multi_levelup_pick.gd 2>&1 | grep -q "^BREACH_MULTI_LEVELUP_PICK_OK"

# Arc-4 PR-#4 P1 review fix regression — Spawner used to set max_hp
# per type but never beam_hp_max, so PRISM beam TTK was a flat 10
# ticks for every enemy (inverted toughness signal).
check-breach-beam-ttk-per-type:
	@$(HEADLESS) --script res://loop/breach/test_breach_beam_ttk_per_type.gd 2>&1 | grep -E "^(  case|BREACH_BEAM_TTK_PER_TYPE_OK|FAIL|ERROR|SCRIPT ERROR)"; \
	$(HEADLESS) --script res://loop/breach/test_breach_beam_ttk_per_type.gd 2>&1 | grep -q "^BREACH_BEAM_TTK_PER_TYPE_OK"

# Arc-4 PR-#4 review fix regression — Batch 1 (P2 #1 + S3 + S4):
# enemy-fired bullets no longer credit recap; armor-mitigated 0-damage
# hits no longer set last_damage_source; take_beam_damage on dying
# enemies is a no-op.
check-breach-review-p2-batch1:
	@$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch1.gd 2>&1 | grep -E "^(  case|BREACH_REVIEW_P2_BATCH1_OK|FAIL|ERROR|SCRIPT ERROR)"; \
	$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch1.gd 2>&1 | grep -q "^BREACH_REVIEW_P2_BATCH1_OK"

# Arc-4 PR-#4 review fix regression — Batch 2 (P2 #2 + P2 #4 + S2):
# mortar reload-bar stamp; pause-before-bail with $HUD absent;
# _stop_beam preserves cooldown timer.
check-breach-review-p2-batch2:
	@$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch2.gd 2>&1 | grep -E "^(  case|BREACH_REVIEW_P2_BATCH2_OK|FAIL —|ERROR — )"; \
	$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch2.gd 2>&1 | grep -q "^BREACH_REVIEW_P2_BATCH2_OK"

# Arc-4 PR-#4 review fix regression — Batch 3 (P2 #3):
# RunRecap now captures APCR everywhere (shells_fired init / dry list /
# resource clause / format() output / build_tag).
check-breach-review-p2-batch3:
	@$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch3.gd 2>&1 | grep -E "^(  case|    sub|BREACH_REVIEW_P2_BATCH3_OK|FAIL —|ERROR — )"; \
	$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch3.gd 2>&1 | grep -q "^BREACH_REVIEW_P2_BATCH3_OK"

# Arc-4 PR-#4 review fix regression — Batch 4 (P2 #5):
# Spawner weight-fallback consistency between total gate + roll.
check-breach-review-p2-batch4:
	@$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch4.gd 2>&1 | grep -E "^(  case|BREACH_REVIEW_P2_BATCH4_OK|FAIL —|ERROR — )"; \
	$(HEADLESS) --script res://loop/breach/test_breach_review_p2_batch4.gd 2>&1 | grep -q "^BREACH_REVIEW_P2_BATCH4_OK"

# Arc-4 PR-#4 Codex P2 review fix regression — Q1ProofRoom enemies are
# spawned before the player; Enemy._ready's cached lookup returned null
# → all enemies inert. Post-pass retro-link via Enemy.set_player.
check-breach-q1-enemy-player-link:
	@$(HEADLESS) --script res://loop/breach/test_breach_q1_enemy_player_link.gd 2>&1 | grep -E "^(  case|BREACH_Q1_ENEMY_PLAYER_LINK_OK|FAIL —|ERROR — )"; \
	$(HEADLESS) --script res://loop/breach/test_breach_q1_enemy_player_link.gd 2>&1 | grep -q "^BREACH_Q1_ENEMY_PLAYER_LINK_OK"

# Arc-4 PR-#4 Codex P1 review fix regression — Q1ProofRoom loadout
# starter reserves. Pre-fix fresh Loadout had 0 he/heat/apcr, so the
# live proof room degraded to AP-only.
check-breach-q1-loadout-seed:
	@$(HEADLESS) --script res://loop/breach/test_breach_q1_loadout_seed.gd 2>&1 | grep -E "^(  case|BREACH_Q1_LOADOUT_SEED_OK|FAIL —|ERROR — )"; \
	$(HEADLESS) --script res://loop/breach/test_breach_q1_loadout_seed.gd 2>&1 | grep -q "^BREACH_Q1_LOADOUT_SEED_OK"

# Arc-4 PR-#4 Codex P1 review fix regression — Depot._is_player detect
# real PlayerTank via switch_archetype signature (live BreachLevel
# delivers an ungrouped, _on_PlayerTank_shoot-less PlayerTank).
check-breach-depot-player-detection:
	@$(HEADLESS) --script res://loop/breach/test_breach_depot_player_detection.gd 2>&1 | grep -E "^(  case|BREACH_DEPOT_PLAYER_DETECTION_OK|FAIL —|ERROR — )"; \
	$(HEADLESS) --script res://loop/breach/test_breach_depot_player_detection.gd 2>&1 | grep -q "^BREACH_DEPOT_PLAYER_DETECTION_OK"

# Arc-4 PR-#4 Codex review fix regression — Batch 5 (P2 #A + P2 #B):
# Enemy.take_damage(0) no-op guard + RunRecap.format() APCR line.
check-breach-review-p2-codex-batch5:
	@$(HEADLESS) --script res://loop/breach/test_breach_review_p2_codex_batch5.gd 2>&1 | grep -E "^(  case|BREACH_REVIEW_P2_CODEX_BATCH5_OK|FAIL —|ERROR — )"; \
	$(HEADLESS) --script res://loop/breach/test_breach_review_p2_codex_batch5.gd 2>&1 | grep -q "^BREACH_REVIEW_P2_CODEX_BATCH5_OK"
