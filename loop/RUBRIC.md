...
**Current state:** 3 — `loop/test_runner.gd` now reports three metrics: `vert_persistence` (raw), `vert_above_floor` (vp - 0.5)/0.5, and `vert_structure_lift` (vp / iid_expected, where iid_expected = Σ p_i² over observed terrain distribution). The refined `structure_lift` decouples spatial structure from terrain concentration. Iter 13 survey (seed 42): default 2.388×, watery 2.357×, fortress 1.529× (lowest!), **biome 2.464× (highest)**, test_p_merge 2.291×. **Surprise findings:** (a) fortress's high raw persistence was *almost entirely* concentration; its true structure is the lowest. (b) biome interpolation generates more structure than either flat endpoint — depth-modulation isn't just shifting distribution, it's adding row-correlated variation. To reach 4: cite a NEW mutation cycle with the refined metric where predicted direction holds. To reach 5: independent-axis demonstration — find a config that is high diversity AND high structure_lift (currently the high-diversity quadrant has medium lift; high-lift quadrant has medium diversity).
