...
**Current state:** 5 — iter 18 constructed `configs/biome_balanced.tres` (default ⇄ `balanced_steel.tres`, both endpoints balanced). High-diversity AND high-structure_lift quadrant filled. **Iter 22 added independent validation via connected-component metric** (`cc_count`, `cc_max`, `cc_avg`): CC ranking is *nearly opposite* to structure_lift ranking (Spearman correlation negative). Fortress: cc_max 256 (giant steel blob), structure_lift 1.751× (worst). biome_balanced: cc_max only 68 (most fragmented blobs), structure_lift 2.628× (best). The two metrics capture **different architectural modes** — "single dominant region" vs "many medium interleaving regions." Anchor 5's load-bearing claim ("Spatial-coherence axis is independent of distribution axis") is now supported by *two independent metrics* showing the same decoupling, not just one.
