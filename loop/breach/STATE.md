...
current_round: 12-open — Round 12 on C9. iter 106 DIAGNOSE → iter 107 SPIKE → iter 108 γ verdict (×42) → iter 109 Gap 2 kill-source (Bullet ×9, Enemy ×3, PlayerTank ×43). C9 lifted 2/5 → 4/5 effective. Next: iter 110 Gap 3 (resource attribution sentence) takes C9 to 5/5 effective.
current_round_phase: BUILD — Round 12 Phase 3/4. Gap 1 wire + Gap 2 kill-source complete; Gap 3 (resource sentence) is the next BUILD, Gap 4 (route diff) optional, Gap 5 (regret quote) deferred to a later round
...
next_action: iter 110 — BUILD — Gap 3 (resource attribution sentence). Per iter-106 diagnosis: extend RunRecap.format()/verdict_sentence with a derived natural-language line that names WHY the resource state mattered, e.g. "You were dry on HE entering BUNKER, where the pressure wants HE blast for the wall density." Logic: when `reserve_left[shell] == 0` AND `killing_band` is one where that shell is the canonical answer, emit the dry-on-X sentence. Use band.canonical_answer string-search for the shell letter (already a SHELL-directive head from iter 108's _canonical_answer_brief helper). The line goes inside the verdict sentence as a NEW PARAGRAPH between "X against pressure." and "(canonical answer: …)". Implementation: add `RunRecap.resource_sentence(canonical_answer) -> String` that returns the dry-on-X clause OR "" when no dry-reserve / canonical mismatch. Modify verdict_sentence to splice it in. Add regression harness test_breach_run_recap_resource_sentence (≥4 assertions: dry-HE-in-HE-band → sentence; dry-HEAT-in-HE-band → no sentence (mismatch); comfortable-HE-in-HE-band → no sentence (not dry); empty canonical → no sentence). Substrate writes: zero new (RunRecap.gd extension only). Hash-anchor verify + test-breach 59 → 60. After Gap 3, C9 effective 5/5; absolute 5/5 still gated on playtest cite per R3. Then Round 12 close: iter 111 META status doc + REVIEW-QUEUE append + bootstrap next round.
score: 49/75 absolute · 49/75 effective  # C1=3,C2=3,C3=4,C4=3,C5=3,C6=3,C7=3,C8=3,C9=4,C10=4,C11=3,C12=3,C13=3,C14=3,C15=4 (iter 108 lifts C9 2→3 via γ verdict sentence; iter 109 lifts C9 3→4 via kill-source attribution from Bullet.source_label propagation)
