...
```yaml
phase: round-24-Q1-proof-room (2 of 4 sprint iters shipped)
iter: 285
...
next_action: iter 286 — BUILD — wire RunRecap route-currency metrics. Now that RunRecap has the storage API (iter 285), iter 286 supplies the caller. Per blueprint deliverable 2 second half: (a) Bullet.gd in `_on_body_entered`: after damaging a body, classify the hit — does the body have an `is_route_gate` group/property? Call player's record_shot_hit(shell_class, "route" or "combat") accordingly. (b) Add `set_meta("is_route_gate", true)` on BrickBlock / SteelBlock instances spawned at gate-row positions when the proof room is active. (c) PlayerTank gets a pass-through `record_shot_hit()` method that delegates to run_recap (loadout-gated). (d) New harness simulates 1 HE shell on a gate-tagged brick → asserts run_recap.shells_spent_on_routes[HE] increments; 1 AP shell on an untagged Light → asserts shells_spent_on_combat[AP] increments. Substrate writes: Bullet.gd (#11; method-existence-gated extension to existing _on_body_entered) + PlayerTank.gd (#52; loadout-gated pass-through method). Hash anchor verification mandatory. Framing-audit gate (PROMPT § iter 283): does this serve user's iter-270 trigger? YES — wiring the diagnostic half makes "shells are route currency" PROVABLE in actual play (not just storage). Gate passes.
