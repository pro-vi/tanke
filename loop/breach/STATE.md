...
```yaml
phase: round-24-Q1-proof-room (1 of 4 sprint iters shipped)
iter: 284
...
substrate_writes_this_arc: 86  # ProceduralLevel.gd ×5 + Bullet.gd ×10 + PlayerTank.gd ×51 + Level.gd + Spawner.gd ×5 + Enemy.gd ×7 (+iter 277 kill-flash, +iter 281 kill-flash ring) + TankSprite.gd + MortarShell.gd
consult_001_progress: 2-of-7-applied  # H5 ribbon-labels (iter 280) + H4 kill-flash-ring (iter 281). Backlog: H6 visibility classes, Q1 breach-economy proof room, Q3 diagnostic room, H1 acceptance gate strengthen, reload bar tank-adjacent dup, Stardew-delta pacing reframe.
...
next_action: iter 285 — BUILD — RunRecap extension: route-currency metrics. Per iter-283-round24-Q1-architect.md deliverable 2 (consult Q3 0.92): "Build one diagnostic room + recap metric: kills by shell, terrain opened by shell, shells spent opening lanes, cards affecting the run."
  (a) Extend RunRecap.gd (arc-4-owned) with fields: `shells_spent_on_routes: Dictionary` by shell class, `shells_spent_on_combat: Dictionary` by shell class, `route_taken: String` (which lane completed first), `time_per_lane: Dictionary`.
  (b) Modify Bullet.gd to tag hits as `was_route_gate: bool` via the BreachBand's gate-cell metadata OR a runtime tag set by the level scene.
  (c) New harness test_breach_run_recap_routes verifies: HE shell on brick gate increments shells_spent_on_routes[HE]; AP shell on Light increments shells_spent_on_combat[AP]; etc.
  (d) Hash anchor preserved (Bullet substrate write may be needed — gate via method-existence check on body OR add an optional param).
Framing-audit-gate check (PROMPT § iter 283): does user's iter-270 "Stardew Valley delta" trigger still map to this iter? YES — RunRecap route-currency metrics are the DIAGNOSTIC half of the consult Q3 verdict 0.92 ("phase passes screen-reading test while failing play test"). The metrics surface whether the player FELT shells as route currency during play. Gate passes.
