...
```yaml
phase: round-24-consult-backlog-applying (Q3 + reload-pip shipped 291-292; H6/H1/Stardew remaining)
iter: 292
...
consult_001_progress: 5-of-8-applied  # H5 ribbon-labels (iter 280) + H4 kill-flash-ring (iter 281) + Q1 proof room sprint (iters 283-290) + Q3 recap-surfacing (iter 291) + reload-pip tank-adjacent dup (iter 292). Backlog (3 remaining): H6 visibility classes (~2-3 iters), H1 acceptance-gate strengthen (~1 iter), Stardew-delta pacing reframe (~30+ iters, needs user scope decision).
...
next_action: iter 293 — apply next consult-001 backlog item. With reload-pip shipped iter 292, remaining 3:
  (a) **H1 acceptance-gate strengthen** (conf 0.86, ~1 iter) — add state→decision second gate to Phase A stranger-on-screen test. Cheapest remaining. Mostly docs + a test-design note in PROMPT.md + the playtest brief.
  (b) **H6 visibility classes** (conf 0.81, ~2-3 iters) — HUD splits into combat-on / conditional / breath-beat; ribbon + route fade except in low-pressure moments. Medium scope; biggest visual change the user would feel.
  (c) **Stardew-delta pacing reframe** (conf 0.87, ~30+ iters) — bigger pivot. Needs user scope decision first.

  Framing-audit gate (PROMPT § iter 283) check: H1 is the only remaining sub-iter-size item; H6 is medium; Stardew-pacing is round-shift requiring user. Three iters of cheap backlog now applied (Q3, reload-pip, will-be H1) — by item (b)/H6 we'll be applying a consult-driven change rather than just docs. Same-family check holds since iters are all anchor-tied.

  Recommended next: (a) H1 acceptance-gate strengthen. Reasoning: cheapest, highest-confidence remaining (0.86). It also closes the playtest-brief's loop by adding a SECOND gate "did visibility change behavior?" to the stranger-on-screen test, which addresses Q3 verdict 0.92 framing concern ("phase can pass screen-reading test while failing play test"). After H1: 6-of-8 consult-001 applied; H6 and Stardew-pacing remain.

  After H1: pause to re-check framing-audit gate. The remaining 2 items are either (a) medium scope (H6) or (b) round-shift requiring user decision (Stardew). The loop should not unilaterally start either without checking the user has scored predictions first, OR explicitly approves the next direction. Likely AskUserQuestion after iter 293 if no user signal.
