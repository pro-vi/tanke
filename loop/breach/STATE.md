...
substrate_writes_this_arc: 68  # ProceduralLevel.gd ×5 + Bullet.gd ×9 + PlayerTank.gd ×45 + Level.gd + Spawner.gd ×5 + Enemy.gd ×4
current_round: 16-closed — Round 16 closed iter 122 META (2-iter round; matches Round-15 brevity). Gap 4 backlog item from iter 106 closed cleanly. No rubric movement. Loop continues default-(D) per PROMPT; iter 123 BUILD-QUALITY Gap 5 regret-quote (last iter-106 backlog item).
current_round_phase: CLOSED — Round 16 complete; iter 123+ continues default-(D) BUILD-QUALITY rotation
...
next_action: iter 123 — BUILD-QUALITY — Round 17 default-(D) scope: **Gap 5 regret-quote** (last iter-106 recap-legibility-deferred backlog item). Per iter-106 §Gap 5: auto-generate a CANDIDATE regret-quote from build_tag + killing_band + reserve_left that gives the player a sharp hypothesis to confirm or deny in the playtest debrief. Render IN PLACE OF the playtest-prompt question (which currently asks "which moment did you regret? right archetype? would switching help?") or as a NEW LINE before it. Implementation: add `RunRecap.regret_quote_candidate(canonical_answer) -> String` returning question-form sentences like "Could Ram have changed BUNKER?" (chassis question) or "Were you HE-heavy in a HEAT band?" (build-vs-pressure question) or "Did the route skip your strength?" (route-vs-build question). Returns "" when state doesn't suggest a sharp question. Wire into PlayerTank breach_prompt build path (same area as iter-121 route-diff). Add regression harness test_breach_run_recap_regret_quote with 4-5 assertions covering each question form. Substrate write ×46 (PlayerTank.gd prompt-build path extension; loadout-gated). Hash-anchor verify + test-breach 63 → 64. Iter 124 META Round 17 close-out.
