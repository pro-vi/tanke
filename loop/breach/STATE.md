...
hash_anchor_at_iter_66: 23d6a2ec3bf2821f  # bit-identical through 38 substrate writes
substrate_writes_this_arc: 38  # ProceduralLevel.gd ×5 + Bullet.gd ×8 + PlayerTank.gd ×20 + Level.gd + Spawner.gd ×4 + Enemy.gd ×1
current_round: 9-open
current_round_phase: BUILD — Round 9e (RAM Tank — collision + melee swing + sprint); blueprint iter-062-round9-architect.md
...
next_action: iter 67 — BUILD — Round 9e: RAM Tank. Read iter-062-round9-architect.md. archetype=RAM: no projectile weapon — damages via collision (driving into an enemy hurts it; smashes brick on collision) + a short-range AoE swing as the fire button (a brief melee cone in the tank's facing direction); special: sprint/dash on demand. PlayerTank.gd (sanctioned). Hash-anchor verify; test-all + test-breach green; a harness for the collision damage + the swing.
...
new_harness_targets: check-breach-{config,shells,depot,he-blast,loadout,depot-choice,level,harness,recap,enemies,assets,armor,dividend,swap,overdrive,hud,apcr,codex,shuffle,depot-roll,rulechangers,stakes,meta,route,xp,ammo,shield,hp,archetype,prism,mortar} + check-silhouette-gate (32 in test-breach aggregate)
...
- 2026-05-22 — **iter 66 (BUILD).** Round 9d — MORTAR Tank: the
  second archetype. archetype=MORTAR = lobbed parabolic shell + AoE
  on impact (~tank-radius), fires over walls (no LoS), slow rate
  (GunTimer 1.5s). New arc-4 MortarShell scene/script (arc via
  sin(πt)·24px; on-impact damages siblings within 18px for 2 dmg).
  PlayerTank.gd substrate write; DEFAULT bit-identical. Hash anchor
  preserved; test-all 5/5, test-breach 32/32 (new check-breach-
  mortar). Δ 0 (C15 at round close). 42/70.

## Next action

**Iter 67 — BUILD — Round 9e: RAM Tank.**
Read `loop/breach/iter-062-round9-architect.md`. archetype=RAM: no
projectile weapon — damages via COLLISION (driving into an enemy
hurts it; smashes brick on collision) + a short-range AoE swing as
the fire button (a brief melee cone in the tank's facing direction);
special ability: sprint/dash on demand. PlayerTank.gd (sanctioned).
Hash-anchor verify; test-all + test-breach green; a harness for the
collision damage + the swing.
