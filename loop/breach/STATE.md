...
substrate_writes_this_arc: 94  # Bullet.gd ×12 + PlayerTank.gd ×59 + Enemy.gd ×8 + BrickBlock.gd ×4 (iter 315 apply_variant_lookup refactor + Loadout brick_variant field plumbing) + others unchanged
hash_anchor_at_iter_313: 23d6a2ec3bf2821f  # bit-identical through 93 substrate writes — BrickBlock.gd variant_texture @export defaults null → _ready branch (`if variant_texture != null:`) never entered on arc-2/3 baseline; sprite stays on sprites_1.png frame 5
hash_anchor_at_iter_315: 23d6a2ec3bf2821f  # bit-identical through 94 substrate writes — BrickBlock.gd apply_variant_lookup() called from _ready; arc-2/3 baseline has no "player" group OR no loadout OR loadout.brick_variant is null → all paths return null → variant_texture stays null → no swap → sprite stays on sprites_1.png frame 5
