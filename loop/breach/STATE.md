...
substrate_writes_this_arc: 60  # ProceduralLevel.gd ×5 + Bullet.gd ×8 + PlayerTank.gd ×41 + Level.gd + Spawner.gd ×4 + Enemy.gd ×2
current_round: 12-open — Round 12 bootstraps on C9 (death attribution / run recap legibility, weakest rubric axis at 2/5). DIAGNOSE iter 106 produced loop/breach/iter-106-c9-diagnose.md — 5 named gaps + 3-iter BUILD plan (iter 107 SPIKE → iter 108-110 BUILD). Root cause: RunRecap.format() unwired from _death_label; the on-screen death panel still shows the arc-2 ASCENDER stat block. Code-review-iter-100 sprint closed iter 105.
current_round_phase: SPIKE — Round 12 Phase 1, iter 107 picks the recap rendering shape (α full-replace / β append / γ sentence) before BUILD commits
...
next_action: iter 107 — SPIKE — Round 12 Phase 1 rendering-shape POCs. Per iter-106 diagnosis, the C9 gap is a UI integration issue, not a data-model issue (RunRecap.gd is already the right shape). The spike runs 3 parallel POCs against the SAME RunRecap data model — no schema changes: (α) full-replace — `_death_label.text` = `run_recap.format()` only (drop ASCENDER stat block; max info, lose arc-2 ritual continuity); (β) append — keep ASCENDER block, append a "WHY" section below from format() with build / killed_by / reserve / band_visits (preserves continuity + adds constraint-6 payload); (γ) sentence — derive a single natural-language regret-quote from build_tag + killing_band + reserve_left, with compact ASCENDER below (highest legibility, hardest to author). Output a SPIKE report (loop/breach/iter-107-c9-spike-report.md) comparing α/β/γ on: info density, arc-2 continuity, sentence-test compatibility, and substrate write count. iter 108 DECISION picks winner + writes blueprint; iter 108-110 BUILD wires it + ships kill-source (Gap 2) + resource sentence (Gap 3). Full plan in loop/breach/iter-106-c9-diagnose.md §3-iter BUILD plan.
