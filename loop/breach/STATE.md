...
```yaml
phase: round-24-Q1-proof-room (4 of 7 sprint iters shipped; plan revised iter 287)
iter: 287
...
next_action: iter 288 — Q1 sprint 5 of 7 (BUILD): playable scene scaffold. Per revised blueprint iter-283-round24-Q1-architect.md: (a) Create scenes/Q1ProofRoom.tscn — a Node2D scene with Camera2D + placeholder spawn anchors. (b) Attach a script that, on _ready, parses Q1ProofRoom.TILE_GRID and instantiates StaticBody2D terrain at each B/S cell (BrickBlock + SteelBlock scenes), Enemy instances at L/H cells (Light/Heavy archetype), and a PlayerTank at the lane the player picks (pick UI deferred to iter 289 OR auto-pick HE for V1). (c) Mark gate-row terrain/enemies with set_meta("is_route_gate", true) so iter-286's wiring fires correctly during play. (d) Verify hash anchor preserved (NEW scene file, no Layer 1/2/3 substrate write). Framing-audit gate (PROMPT § iter 283): does this serve user's iter-270 trigger? YES — the scene is the artifact the user can actually playtest. Gate passes.
