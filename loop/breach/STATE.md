...
```yaml
phase: round-24-Q1-proof-room (5 of 7 sprint iters shipped — PLAYABLE)
iter: 288
...
next_action: iter 289 — Q1 sprint 6 of 7 (BUILD): per-lane playthrough harness. Per revised blueprint: drive a virtual player through each of the 4 lanes via scripted movement + fire; assert gate clearance times match the design property (HE lane ≤ 3s with HE shell, ≥ 25s with AP; APCR lane IMPASSABLE without APCR; HEAT lane 2× faster with HEAT vs. AP; AP lane completable). Tests the end-to-end claim "shells are route currency, not just damage flavor" in REAL playable scene timings — not just design assertions. The harness should also verify route-currency metrics fire: post-playthrough, run_recap.shells_spent_on_routes[HE] > 0 after the HE lane playthrough. Framing-audit gate: continues to serve user's iter-270 trigger; this is the closing verification before the user takes over playtest. Gate passes.
