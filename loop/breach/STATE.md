...
```yaml
phase: round-24-consult-backlog-applying (Q3 recap shipped iter 291; H6/H1/reload-dup/Stardew remaining)
iter: 291
...
substrate_writes_this_arc: 90  # Bullet.gd ×12 + PlayerTank.gd ×53 (+iter 291 Q3 recap surfacing) + Enemy.gd ×7 + others unchanged
consult_001_progress: 4-of-8-applied  # H5 ribbon-labels (iter 280) + H4 kill-flash-ring (iter 281) + Q1 proof room sprint (iters 283-290) + Q3 recap-surfacing (iter 291). Backlog (4 remaining): H6 visibility classes (~2-3 iters), H1 acceptance-gate strengthen (~1 iter), reload bar tank-adjacent dup (~1-2 iters), Stardew-delta pacing reframe (~30+ iters, needs user scope decision).
...
next_action: iter 292 — pick next consult-001 backlog item OR await user playtest scoring. Q3 recap shipped iter 291; remaining 4:
  (a) **H1 acceptance-gate strengthen** (conf 0.86, ~1 iter) — add state→decision second gate to Phase A stranger-on-screen test. Cheapest remaining. Mostly docs + a test-design note in PROMPT.md or the brief.
  (b) **reload bar tank-adjacent dup** (conf 0.84, ~1-2 iters) — add tiny tank-adjacent reload pip alongside top-left bar; combat-timing UI near combat focus.
  (c) **H6 visibility classes** (conf 0.81, ~2-3 iters) — HUD splits into combat-on / conditional / breath-beat; ribbon + route fade except in low-pressure moments. Medium scope.
  (d) **Stardew-delta pacing reframe** (conf 0.87, ~30+ iters) — bigger pivot. Needs user scope decision first.

  Framing-audit gate (PROMPT § iter 283) check: each remaining candidate addresses a consult-001 verdict and is downstream of Option B from iter 283. Item (a) is the cheapest + has the highest unclamed-confidence remaining (0.86). Item (b) is also cheap (0.84). Both close gaps cleanly. Item (c) has a higher cost but is the most visible HUD change the user would feel.

  Recommended next: (b) reload bar tank-adjacent dup. Reasoning: consult-001 said "combat-timing UI belongs near the player's gaze" (conf 0.84); the top-left bar passes the stranger test but consult predicts it'll be IGNORED during pressure. A tank-adjacent pip directly tests that prediction without committing to the full H6 visibility-class refactor.

  Alternative: HALT pending user playtest scoring of consult-001's 3 predictions. If loop continues without user scoring, predictions stay UNTESTED and feel_consult_cap stays at 3 forever.
