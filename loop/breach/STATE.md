...
```yaml
phase: round-24-Q1-CLOSED-awaiting-user-playtest-scoring
iter: 290
...
hash_anchor_at_iter_281: 23d6a2ec3bf2821f  # bit-identical through 86 substrate writes — Enemy.gd _spawn_kill_flash_ring (consult-001 H4 fix) gated on `_last_damage_shell >= 0`; legacy / arc-2/3 path stays at 1 ColorRect, ring edges never spawn
hash_anchor_at_iter_286: 23d6a2ec3bf2821f  # bit-identical through 88 substrate writes — Bullet.gd _try_record_shot_hit (#11) + PlayerTank.gd record_shot_hit pass-through (#52); both gated (Bullet via method-existence on player, PlayerTank via run_recap != null); arc-2/3 procedural baseline never has loadout/run_recap → silent path
...
substrate_writes_this_arc: 89  # Bullet.gd ×12 (+iter 289 APCR-steel route fix) + PlayerTank.gd ×52 + others unchanged
consult_001_progress: 3-of-8-applied  # H5 ribbon-labels (iter 280) + H4 kill-flash-ring (iter 281) + Q1 proof room sprint CLOSED at iter 290 (full 7-iter program iters 283-290). Backlog (5 items, total reframed): H6 visibility classes, H1 acceptance-gate strengthen, reload bar tank-adjacent dup, Q3 diagnostic recap surfacing, Stardew-delta pacing reframe. (Counting Q1 as 1 applied item per recommendation; total recommendations now 8 with Q3 recap-surfacing split out as separate from Q1's data layer.)
...
next_action: iter 291 — pick from consult-001 backlog OR await user direction. Q1 sprint closed; the 5 remaining backlog items from consult-001 (uncalibrated cap=3):
  (a) **H6 visibility classes** (conf 0.81, ~2-3 iters) — split HUD into combat-on / conditional / breath-beat; ribbon + route fade except in low-pressure moments
  (b) **reload bar tank-adjacent dup** (conf 0.84, ~1-2 iters) — add tiny tank-adjacent reload pip alongside the top-left bar
  (c) **Q3 diagnostic recap surfacing** (conf 0.92, ~2 iters) — surface RunRecap.shells_spent_on_routes / time_per_lane on the death/exit screen so the route-currency data the loop already stores reaches the player
  (d) **H1 acceptance-gate strengthen** (conf 0.86, ~1 iter) — add a state→decision second gate to the Phase A stranger-on-screen test
  (e) **Stardew-delta pacing reframe** (conf 0.87, ~30+ iters) — bigger pivot per consult Q "Stardew was about pacing/rhythm/economy, not HUD"; needs user scope decision first

  Framing-audit gate (PROMPT § iter 283) check for next iter: each candidate addresses a consult-001 verdict and is downstream of the same user direction (Option B at iter 283). Cheapest: (b) reload bar tank-dup at conf 0.84; second cheapest with biggest payoff: (c) Q3 recap surfacing at conf 0.92 (makes the route-currency data we just shipped VISIBLE to the player — closing the iter-285-289 loop). Recommended next: (c) — highest confidence + biggest leverage given Q1 sprint just landed and we want playtest signals to surface.

  Alternative: HALT pending user scoring of consult-001's 3 predictions before continuing. If the loop continues without user scoring, predictions stay UNTESTED and the cap stays at 3 forever. User can short-circuit by writing playtest / halt / stop.
  ...
  (d) **Q1 pivot** (3-5 iters, conf 0.90) — pre-Phase-B breach-economy proof room. Bigger pivot — likely needs user direction since it changes Phase B's scope.
  (e) **Q3 diagnostic room + recap metric** (2-3 iters, conf 0.92) — instrument kills-by-shell, terrain-opened-by-shell, shells-spent-opening-lanes; surface in RunRecap.
  Recommended order: (a) pickup-toast next (paired with H5; closes the H5 recommendation set) → (b) reload bar dup → (c) H6 visibility classes → (d/e) decided by user direction. The architectural Q1 pivot is the consult's strongest finding (0.90) but also the biggest scope shift; queueing user input at REVIEW-QUEUE before unilateral pivot.
