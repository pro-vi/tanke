...
current_round: 12-open — Round 12 on C9 (death attribution / run recap legibility, 2/5). iter 106 DIAGNOSE produced surface map (5 gaps + 3-iter BUILD plan). iter 107 SPIKE compared 3 rendering shapes (α full-replace / β append / γ sentence) — recommendation γ unlocks `BreachBand.canonical_answer` surfacing for diagnostic-grade recap. Next: iter 108 DECISION + BUILD (γ implementation).
current_round_phase: DECISION-pending — iter 107 SPIKE concluded, iter 108 affirms γ (or falls back to β) and BUILDs Gap 1 wire
...
next_action: iter 108 — DECISION + BUILD — affirm γ recommendation (or fall back to β with a noted reason) and ship Gap 1 wire. Implementation per iter-107-c9-spike-report.md §Authoring spec: (1) add `RunRecap.verdict_sentence(canonical_answer: String = "") -> String` + helpers `_format_resource_clause()` / `_pressure_first_phrase()` / `_canonical_answer_brief()` to scripts/RunRecap.gd; (2) PlayerTank.gd substrate write ×42 — replace the `_death_label.text = "YOU DIED\n\nDEPTH %d…"` line with `"YOU DIED" + verdict + compact_footer` where verdict reads `band.canonical_answer` from the killing band (defensively null-checked); (3) regression harness `test_breach_run_recap_verdict_sentence` with ≥4 assertions: standard shape, resource-clause for low-vs-comfortable reserves, canonical-skip when band has no answer, long-pressure truncation; (4) hash-anchor verify + test-all + test-breach (57 → 58); (5) C9 lifts to 3/5 effective (4/5+ unlocks at Gap 2/3); (6) ScheduleWakeup iter 109 (Gap 2 — kill source tracking).
