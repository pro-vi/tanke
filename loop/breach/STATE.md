...
```yaml
phase: round-24-Q1-proof-room (6 of 7 sprint iters shipped — runtime VERIFIED)
iter: 289
...
next_action: iter 290 — Q1 sprint 7 of 7 (CLOSE — REVIEW): playtest brief + REVIEW-QUEUE close + CONSULT-LEDGER prediction-scoring trigger.
  (a) Write loop/breach/Q1-PROOF-ROOM-PLAYTEST-BRIEF.md — 1-page playtest spec for the user. Includes: how to launch (godot --path . scenes/Q1ProofRoom.tscn), the 4-lane structure, the 3 falsifiable consult-001 predictions, what to look for, debrief template.
  (b) REVIEW-QUEUE entry #30 — Q1 sprint shipped (7 iters 283-289 + brief 290), awaits user scoring of consult-001 predictions. CONSULT-LEDGER consult-001 § Affected anchors gets prediction-scoring trigger (post-playtest, user marks each of 3 predictions hit/partial/miss; STATE.consult_calibration updates; per CONSULT-LEDGER rules ≥2 hits + hit_rate ≥50% raises feel_consult_cap to 4).
  (c) No substrate touch (META iter — docs + REVIEW-QUEUE).
  Framing-audit gate (PROMPT § iter 283): final check — does the brief give user a concrete way to score consult-001's predictions against real play? YES — that's its sole purpose. Gate passes; sprint closes.
