...
substrate_writes_this_arc: 64  # ProceduralLevel.gd ×5 + Bullet.gd ×9 + PlayerTank.gd ×43 + Level.gd + Spawner.gd ×4 + Enemy.gd ×3
current_round: 13-open — Round 13 bootstraps on C8 (Sentence test compliance, 3/5). iter 112 DIAGNOSE audited the 12-upgrade catalog (9 strict-PASS, 3 MARGINAL inventory-scaling) + 5-band coverage (3 covered, 2 gaps: tutorial_choke + open_killbox). Lift plan: OPTION B — add SCOUT_TELEGRAPH (tutorial_choke) + SNAP_TURRET (open_killbox) sentence-test-compliant upgrades. Full diagnosis: loop/breach/iter-112-c8-c1-diagnose.md. C1 re-score: stays at 3/5 (anchors 4+ playtest-only).
current_round_phase: DIAGNOSE-complete — iter 112 audit + lift plan published; iter 113 DECISION + BUILD on OPTION B (add SCOUT_TELEGRAPH + SNAP_TURRET upgrades to close the 2-band gap)
...
next_action: iter 113 — DECISION + BUILD — Round 13 Phase 2: implement OPTION B from iter-112 diagnosis. Add two new sentence-test-compliant upgrades to Depot.gd to close the 2-band coverage gap. (1) `SCOUT_TELEGRAPH` for tutorial_choke — incoming Light spawns get a 0.5s pre-spawn marker on the route strip (or HUD overlay); sentence: "helps me climb through tutorial_choke by changing how I anticipate light scouts." (2) `SNAP_TURRET` for open_killbox — facing rotation is instant for the next 8s after pick (or per-band-entry); sentence: "helps me climb through open_killbox by changing how I rotate." Wire each: enum value + label + apply_upgrade routing + depot pool eligibility (likely band-gated). Substrate writes: 0-1 (Depot.gd is arc-4-owned; PlayerTank.gd ×44 only if SCOUT_TELEGRAPH needs a HUD route-strip hook for the pre-spawn marker). Add regression harness test_breach_band_targeted_upgrades.gd asserting each new UpgradeKind passes sentence-test, applies to Loadout cleanly, surfaces in _label_for_kind, and (for SCOUT_TELEGRAPH) actually hooks the HUD. Hash-anchor verify + test-breach 60 → 61. After iter 113, C8 anchor 3 cleanly satisfied → 3/5 → 4/5 effective (anchor 4 cognitive-max).
score: 47/75 absolute · 48/75 effective  # C1=3,C2=3,C3=4,C4=3,C5=3,C6=4eff/3abs,C7=3,C8=3,C9=2,C10=4,C11=3,C12=3,C13=3,C14=3,C15=4  (iter 111 META correction per RUBRIC.md: Round 12 work shipped IS recap legibility, which is **C6 Death attribution**, not C9 Identity. C6 effective lifts 3→4 via anchor-4 cognitive-max — the verdict_sentence + canonical_answer + kill_source + resource_sentence chain structurally demonstrates "reads as actionable diagnosis." C6 absolute stays 3 — anchor 4 is [FEEL]; absolute 4 needs playtest cite. C9 stays at 2 — no Identity-axis work shipped. See round-12-summary.md §Scoring correction.)
