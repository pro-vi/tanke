...
substrate_writes_this_arc: 66  # ProceduralLevel.gd ×5 + Bullet.gd ×9 + PlayerTank.gd ×43 + Level.gd + Spawner.gd ×5 + Enemy.gd ×4
current_round: 14-open — Round 14 bootstraps from iter-115 structural-ceiling audit. 8 of 8 audited 3/5 axes at structural ceiling (anchors 4-5 [FEEL] playtest-only). Cleanest remaining BUILD-able surface: open_killbox C8 completion via chassis mechanic. Recommendation: REAR_GUARD (passive auto-defense in rear 90° cone). Iter 116 DECISION + BUILD; iter 117 META close-out. Forward-path ceiling without playtest signal: 49 → ~51 effective.
current_round_phase: DIAGNOSE-complete — iter 115 structural-ceiling audit + REAR_GUARD recommendation published; iter 116 DECISION + BUILD on REAR_GUARD (open_killbox C8 anchor-3 completion).
...
next_action: iter 116 — DECISION + BUILD — Round 14 Phase 2: implement REAR_GUARD per iter-115 audit recommendation. Closes the open_killbox C8 anchor-3 gap deferred from Round 13 with a passive chassis-mechanic upgrade: when owned, an AP shot auto-fires at the closest enemy in the rear 90° cone (range ~96px) when one enters it; costs no shell; cooldown ~2.5s. Sentence: "helps me climb open_killbox by changing how I commit to facing — rear scouts no longer demand a turn." Implementation: (a) Loadout flag `has_rear_guard: bool`; (b) Depot UpgradeKind value (catalog 13 → 14; pool entry; label); (c) PlayerTank.gd substrate write ×44 — per-frame scan of "enemy" group for any body in the rear 90° cone within 96px, fires AP backward (180° from current direction) via shoot signal; cooldown timer; (d) test_breach_overdrive + test_breach_meta updated for catalog+pool sizes; (e) NEW test_breach_rear_guard.gd: 5+ assertions covering flag default + apply + rear-cone detection + fire fires + cooldown + front-cone-no-fire regression. Hash-anchor verify + test-breach 61 → 62. After iter 116, C8 absolute 3 → 4 (anchor 3 fully met — all 5 bands covered); effective stays 4 (already lifted iter 113). Iter 117 META Round 14 close-out.
score: 48/75 absolute · 49/75 effective  # C1=3,C2=3,C3=4,C4=3,C5=3,C6=4eff/3abs,C7=3,C8=4eff/3abs,C9=2,C10=4,C11=3,C12=3,C13=3,C14=3,C15=4  (iter 113 lifts C8 3→4 effective via SCOUT_TELEGRAPH closing the tutorial_choke band-pressure gap; C8 absolute stays 3 — anchor 4 [FEEL] needs playtest cite; open_killbox gap deferred.)
